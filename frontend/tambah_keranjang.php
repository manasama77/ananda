	...
	if($row_event_list > 0){ // Event Start
		...
		if($l == "true"){
			...
			if($row > 0){
				...
				$q_insert = mysqli_query($con, "INSERT INTO tbl_list_penjualan (id_list_penjualan, kode_penjualan, id_produk, nama_produk, satuan, qty, hpj, total_harga, status, id_create, date_create, jenis_pelanggan, kode_member, nama_member) VALUES ('', '$a', '$b', '$c', '$d', '$e', '$f', '$g', '$status', '$h', now(), '$i', '$j', '$k')");
			}
		}
